...
function cargarInsignias() {
    ...
    const todas =
        JSON.parse(localStorage.getItem("insignias3ESO")) || {};
...
function activarEtiquetas() {
    ...
    etiquetas.forEach(et => {
        ...
        const nuevaEtiqueta = et.cloneNode(true);
        ...
        nuevaEtiqueta.draggable = true;

        // -------------------------
        // PC: DRAG
        // -------------------------
        nuevaEtiqueta.addEventListener(
            "dragstart",
            e => {

                if (
                    nuevaEtiqueta.classList.contains("usada") ||
                    casoCompletado
                ) {
                    e.preventDefault();
                    return;
                ...
                etiquetaActual =
                    nuevaEtiqueta.dataset.etiqueta;

                nuevaEtiqueta.classList.add(
                    "seleccionada"
                );
        ...
        nuevaEtiqueta.addEventListener(
            "dragend",
            () => {
                nuevaEtiqueta.classList.remove(
                    "seleccionada"
                );
            }
        );
...
//   COMPROBAR RESPUESTA
// =========================
function comprobarRespuesta(bloque, etiqueta) {

    if (!etiqueta || casoCompletado) {
        return;
    }

    // Si el bloque ya está correctamente resuelto,
    // no se vuelve a puntuar.
    if (bloque.classList.contains("correcto")) {
        return;
    }

    const correcto = bloque.dataset.correcto;
    ...
    if (
        ...
        bloque.classList.remove("incorrecto");
        bloque.classList.add("correcto");

        // Evita duplicar la etiqueta dentro del bloque
        const colocadaExistente =
            bloque.querySelector(".colocada");

        if (!colocadaExistente) {

            const colocada =
                document.createElement("div");

            colocada.classList.add("colocada");

            colocada.innerText = etiqueta;

            bloque.appendChild(colocada);
        }

        puntuacion += 10;

        // Marcar la etiqueta como utilizada
        const etiquetaDOM =
            document.querySelector(
                `.etiqueta[data-etiqueta="${etiqueta}"]`
            );

        if (etiquetaDOM) {
            etiquetaDOM.classList.remove(
                "seleccionada"
            );

            etiquetaDOM.classList.add("usada");

            etiquetaDOM.draggable = false;
        }
        ...
        bloque.classList.remove("correcto");
...
function comprobarActividad() {
    ...
    bloques.forEach(b => {
        ...
        if (b.classList.contains("correcto")) {
            completos++;
        }
    });

    // Solo se completa cuando TODOS
    // los bloques están correctamente resueltos.
    ...
    // Insignia de caso completado
    desbloquearInsignia("casoCompletado");

    // Perfecto = 5 aciertos de 10 puntos
    if (puntuacion === 50) {
        desbloquearInsignia("perfecto");
    }

    // -------------------------
    // CASOS DISTINTOS COMPLETADOS
    // -------------------------
    const datos =
        JSON.parse(
            localStorage.getItem("resultados3ESO")
        ) || [];

    const casosCompletados =
        new Set(
            datos
                .filter(r => r.alumno === alumno)
                .map(r => r.caso)
        );

    if (casosCompletados.size >= 3) {
        desbloquearInsignia("tresCasos");
    }

    if (casosCompletados.size >= 10) {
...
function guardarResultado() {
    ...
    // Evitar guardar el mismo caso
    // varias veces durante la misma sesión.
    const yaGuardado = datos.some(
        r =>
            r.alumno === alumno &&
            r.caso === casoActual.titulo &&
            r.fechaSesion === window._sesionCaso
    );
    ...
    datos.push({
        ...
        fechaSesion: window._sesionCaso
    });
...
function exportarInsignias() {
    ...
    document.getElementById("expNombre").innerText =
        alumno;

    document.getElementById("expCurso").innerText =
        curso;

    const cont =
        document.getElementById("expInsignias");
    ...
    const panel =
        document.getElementById("exportarImagen");
...
document.addEventListener(
    ...
    () => {
        ...
        // Generar un identificador para cada
        // nueva ejecución de la página.
        window._sesionCaso = null;
    }
);